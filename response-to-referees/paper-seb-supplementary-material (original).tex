...
Section \ref{over-fit} of the main paper looks like part of an ordinary paper but in fact it was generated by a \emph{Mathematica\/} program. The section looks like typical text from a paper, but it was data generated by a program.
...
The idea is very simple, but very effective. The ``code'' for this paper includes the \emph{Mathematica\/} notebook that generated section \ref{over-fit}. In fact, the notebook is written as a self-contained report or paper, as \emph{Mathematica\/} notebooks generally are, and it thoroughly documents how it works. 
...
David Spiegelhalter is concerned how statistics is often misused and misunderstood. In his \emph{The Art of Statistics\/} \cite{Spiegelhalter} Spiegelhalter brings together his advice for making reliable statistical claims: they need to be accessible, intelligible, assessable, and usable --- the claims need to be properly accountable. Spiegelhalter proposes ten questions to ask when confronted with any claim based on statistical evidence. Some of his questions are quite general, and might be applied to any sort of scientific claims, but all have analogous questions that could be addressed to software code or papers relying on code --- analogues are suggested in \textbf{bold} below. 
...
The main paper focuses on selected epidemiological papers and their computational models. Are the issues illustrated by these case studies typical of computational science more broadly? We undertook, first, some selected studies of papers in different fields, then undertook a randomized, stratified pilot survey covering several leading peer reviewed journals.
...
Moreover, this paper follows the more general \RAPstar\ methodology. For example, as standard practice, a Unix makefile is used to make it easy to analyze and generate all data, typeset the paper, and more. The following options are provided:

\vskip 1ex{\small \begin{center}
\input generated.make-help.tex
\end{center}
}\vskip 1ex

Conforming to the \RAPstar\ methodology, the summary above (including all the \LaTeX\ typesetting details) was generated automatically (in fact, by just saying ``\texttt{make data}'', which also generated all the other data). However, when \texttt{make help} or \texttt{make help-brief} are run, they generate plain text summaries without any of the \LaTeX\ typesetting details used here (e.g., \LaTeX\ will appear simply as \texttt{Latex}). 

The \RAPstar\ approach cannot enforce the truth of the summary, but it very significantly increases the chances that the option summary is correct and up to date. For example, if refactoring leads to an option being deleted, then it will also disappear \emph{with no further work\/} from the table above. Also, since the summary was proof-read at the same time as proof-reading this \supplement, following \RAPstar\ also increases the chances that any errors or functionality omissions or issues in the makefile have been detected and corrected.
